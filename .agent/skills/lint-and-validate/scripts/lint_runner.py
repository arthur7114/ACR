...
def detect_project_type(project_path: Path) -> dict:
    ...
    if package_json.exists():
        ...
        try:
            ...
            if "lint" in scripts:
                result["linters"].append({"name": f"{pkg_mgr} lint", "cmd": [pkg_mgr, "run", "lint"]})
            elif "eslint" in deps:
                runner = "npx" if pkg_mgr == "npm" else (pkg_mgr if pkg_mgr in ["pnpm", "bun"] else "yarn")
                result["linters"].append({"name": "eslint", "cmd": [runner, "eslint", "."]})
            
            # Check for TypeScript
            if "typescript" in deps or (project_path / "tsconfig.json").exists():
                runner = "npx" if pkg_mgr == "npm" else (pkg_mgr if pkg_mgr in ["pnpm", "bun"] else "yarn")
                tsc_cmd = [runner, "tsc", "--noEmit"] if runner == "npx" else [runner, "exec", "tsc", "--noEmit"] if runner == "pnpm" else [runner, "tsc", "--noEmit"]
                result["linters"].append({"name": "tsc", "cmd": tsc_cmd})
