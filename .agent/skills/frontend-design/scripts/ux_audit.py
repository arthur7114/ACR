...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_form = bool(re.search(r'<form|<input\b|<textarea\b|<select\b', content, re.IGNORECASE))
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', '.claude', '.codex', 'dist', 'build', '.next'}]
            for file in files:
