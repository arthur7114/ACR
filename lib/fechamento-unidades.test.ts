...
const INTERMEDIACOES_GM2_AGOSTO = [
  { tipo: "intermediacao" as const, apto: "3" },
  { tipo: "intermediacao" as const, apto: "8" },
  { tipo: "intermediacao" as const, apto: "23" },
]
...
test("GM II ago/2026: apto listado na tabela de intermediações é intermediação, não inadimplente", () => {
  const unidades = criarClassificadorUnidades(aptosDeIntermediacao(INTERMEDIACOES_GM2_AGOSTO))
...
test("GM II ago/2026: tiles contam 3 intermediações e 2 inadimplentes", () => {
  ...
  assert.equal(
    contarUnidadesIntermediadas(LINHAS_GM2_AGOSTO, INTERMEDIACOES_GM2_AGOSTO, unidades),
    3,
  )
...
test("intermediação sem linha na vigência ainda é contada uma vez", () => {
  const intermediacoes = [{ tipo: "intermediacao" as const, apto: "204" }]
  const linhas = [linha({ apto: "101", inquilino: "LOCATÁRIO", aluguel: 700, total: 700 })]
  const unidades = criarClassificadorUnidades(aptosDeIntermediacao(intermediacoes))

  assert.equal(contarUnidadesIntermediadas(linhas, intermediacoes, unidades), 1)
})

test("intermediação sem apto informado não some da contagem", () => {
  const intermediacoes = [{ tipo: "intermediacao" as const, apto: null }]
  const linhas = [linha({ apto: "101", inquilino: "LOCATÁRIO", aluguel: 700, total: 700 })]
  const unidades = criarClassificadorUnidades(aptosDeIntermediacao(intermediacoes))
