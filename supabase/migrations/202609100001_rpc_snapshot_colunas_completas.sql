-- Reconcilia as DUAS vias de gravacao do snapshot mensal com o tipo
-- `IndicadoresSnapshotRow`. Cada via perdia uma coluna diferente, e o
-- ON CONFLICT mascarava o defeito: linha ja existente conservava o valor
-- antigo, entao so um INSERT novo -- fechamento de competencia inedita --
-- revelaria a perda.
--
-- 1. `persistir_pacote_fechamento_v1` (processamento): a migration
--    202609020001 recriou a funcao a partir da versao ANTERIOR a
--    202608270003 e trocou `garagem_recebida` por `observacao`. Sem a
--    garagem, o gap de inadimplencia por componentes cai no fallback por
--    aluguel (lib/indicadores-aggregation.ts), porque as bases deixam de
--    ser compativeis.
-- 2. `aplicar_reparo_indicadores_v4` (reparo/backfill): nunca recebeu
--    `observacao`, adicionada depois em 202609020001. Sem ela o mapa de
--    riscos perde o texto que explica a celula no hover.
--
-- Nenhuma mudanca de assinatura, semantica ou tipo -- apenas as colunas que
-- faltavam em cada lado. Corpos gerados a partir das duas definicoes
-- vigentes, para nao repetir a copia manual que originou a regressao.
-- Guarda permanente: lib/server/indicadores-snapshots-rpc.test.ts compara a
-- lista de colunas destas RPCs com as chaves de IndicadoresSnapshotRow.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_snapshots, '[]'::jsonb)) loop
    ...
    insert into public.imovel_competencias (
      ...
      calculo_versao, checksum, observacao
    ) values (
