-- Guarda a observação da(s) linha(s) da prestação no snapshot mensal. O mapa
-- de riscos precisa dela para explicar a célula no hover: rescisão
-- ("RESCISÃO. PROPORCIONAL DE 01 DIA (01/06)"), inadimplência, acordo. Coluna
-- aditiva e nula; a RPC de persistência passa a gravá-la. Chamadores antigos
-- (payload sem o campo) continuam válidos.
alter table public.imovel_competencias
  add column if not exists observacao text;

comment on column public.imovel_competencias.observacao is
  'Observação consolidada das linhas da prestação para a unidade na competência (texto do documento).';
