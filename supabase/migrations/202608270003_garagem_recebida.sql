-- Persiste a garagem recebida por unidade (base compativel para o gap de
-- inadimplencia por componentes, P0.4). Coluna aditiva + RPC atualizada.

alter table public.imovel_competencias
  add column if not exists garagem_recebida numeric;

comment on column public.imovel_competencias.garagem_recebida is
  'Garagem recebida na competencia, separada do aluguel. Null = nao observado (snapshots antigos); o gap por componentes so usa cobranca_esperada quando este campo e conhecido ou nada foi recebido.';

-- RPC atualizada para gravar garagem_recebida (alem de eventos e cobranca_esperada).
