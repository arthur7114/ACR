...
- CA14.8.1 (2026-08-31): o desdobramento de despesas usa o `tipo` estruturado do documento de despesas quando ele existe (`energia`, `agua`, `iptu`, `seguro`), e só recorre à inferência por texto quando não existe. A observação de uma despesa é texto de nota fiscal ou boleto e frequentemente não cita a categoria — no GM II julho a segunda ENEL (R$ 106,43, observação "Comprovante de pagamento") e os 8 seguros ("Boleto com Nosso Número...") caíam em "Outros", somando R$ 1.227,76 fora de categoria, enquanto a primeira ENEL acertava só porque a observação dizia "Pix". Correto: Energia R$ 211,18 (2 itens), Água e esgoto R$ 1.827,90, Seguros R$ 1.121,33 (8 itens).
...
- CA-IND18: quando o toggle de “Riscos por imóvel” está em Inadimplência, um painel de resumo antecede o mapa mês a mês, respondendo diretamente: inadimplência do mês (não pago na competência selecionada), inadimplência acumulada (dívida de meses anteriores) e o total em aberto dos dois somados — sem substituir um pelo outro. Abaixo, lista apenas as unidades inadimplentes na competência selecionada, cada uma com situação atual, os meses em que ficou inadimplente (dentro da janela visível do mapa) e o valor em aberto; unidades que já quitaram não aparecem na lista mesmo tendo meses inadimplentes no histórico. Clicar numa unidade rola até a linha correspondente no mapa abaixo. Nenhum valor é inventado: se faltar o aluguel esperado de alguma unidade inadimplente, o total do mês aparece como `—`, não como soma parcial.
...
- CA-IND16: o reparo César Rêgo vincula as linhas históricas aos cadastros canônicos existentes e envia à RPC v4 somente receitas `receita_aluguel` e despesas `rateio_ted`, junto dos snapshots, validações e auditoria, sob concorrência otimista e na mesma transação. Qualquer fechamento incompleto bloqueia todo o commit selecionado. Após o reparo, não há receita César Rêgo sem vínculo válido nem cadastro no empreendimento errado.
...
- CA-IND19: a Evolução mensal inicia com as últimas 12 competências disponíveis e permite atalhos de 3, 6 e 12 meses ou intervalo personalizado inclusivo entre competências processadas. A escolha persiste na URL, não altera a competência nem os KPIs do topo e, em telas estreitas, os controles se reorganizam sem criar overflow horizontal da página; somente o gráfico pode rolar internamente quando necessário.
- CA-IND20: em Riscos por imóvel, a célula de cada unidade exibe o inquilino do snapshot da própria competência, nunca o inquilino atual como fallback. Nome ausente aparece como “Inquilino não informado”; nomes longos são truncados visualmente, mas permanecem completos no título e no nome acessível. Quando o empreendimento possui uma única unidade, a linha agregada também expõe o inquilino mensal sem exigir expansão.
...
- CA14.17: os rechecks `acordos_competencias` e `duplicate_agreement_payment` nunca populam `actual`/`expected` (campos monetários, formatados como R$ na lista de pendências e no modal) com a contagem de acordos/rescisões; a contagem aparece só no texto da mensagem. Sem valor a decidir, resolver a pendência oferece apenas "Ignorar pendência".
...
- CA27.7 (reunião de 2026-08-27): em acordo, rescisão e atraso a base comissionável é o TOTAL pago pelo inquilino (aluguel + garagem + água + IPTU), porque a comissão retida ali é a taxa de administração — jul/2026: 7% em Grand Castelão I, GM II e LOC MAIS; 6% em GM I. Só a intermediação usa aluguel + garagem como base. Calcular o percentual do acordo sobre a base menor inflava o valor exibido (7,51% em vez de 7,00%).
- CA27.8 (reunião de 2026-08-27): fechamento com repasse embutido no próprio documento (extrato consolidado César Rêgo/Plural) sai do denominador de comprovantes — comprovante não aplicável não é comprovante ausente, e não gera alerta de incompletude para José Walter nem Galpão Pompílio Gomes.
...
- CA-IND22 (revisado em 2026-08-28): a inadimplência acumulada nunca é exibida como R$ 0,00 quando o documento da competência não possui a seção de dívidas acumuladas. O extrato consolidado (César Rêgo/Plural — João Cordeiro, Galpão Pompílio Gomes, Galpão José Walter) não traz essa seção: a métrica fica `desconhecida` ("—") e o fechamento aparece nominalmente na lacuna `inadimplencia_nao_extraida`. Lista vazia com a seção identificada continua sendo zero confirmado (ex.: Grand Castelão maio). A ausência é detectada por `campos_ausentes` ou por `plano_extracao.secoes_identificadas` sem menção a inadimplência, o que corrige a exibição sem exigir reprocessamento.
- CA-IND22.1: quando o documento não traz a seção, a dívida acumulada é DERIVADA do histórico mensal do próprio sistema — soma dos gaps das competências anteriores (`cobranca_esperada` ou `aluguel_esperado` menos o aluguel recebido NA competência) menos os atrasos já recuperados, com piso em zero. A precedência é: seção do documento, depois histórico derivado, depois desconhecida.
- CA-IND22.2: a derivação é fail-closed quanto à base. `aluguel_competencia` nulo significa "não observado": cair em `aluguel_recebido` misturaria atraso recuperado com aluguel do mês e subtrairia o atraso duas vezes (produzindo saldo negativo). Se qualquer mês da janela não separar as duas grandezas, a métrica permanece desconhecida em vez de exibir um número derivado de bases incompatíveis.
...
- CA-IND25 (definição contratual): "aluguel médio" é ambíguo e não deve aparecer sem qualificação. Na Revisão a métrica é **aluguel recebido médio**, com numerador igual ao aluguel recebido no mês e denominador igual às unidades com cobrança ativa (alugadas + inadimplentes); a unidade inadimplente participa com recebimento zero em vez de sair da conta. Dividir apenas pelas unidades que receberam descrevia a média de quem pagou e escondia a parcela da carteira sem recebimento (João Cordeiro julho: R$ 1.237,05 com uma das duas unidades sem pagar; o correto é R$ 618,53 por unidade com cobrança ativa). "Aluguel contratado médio" é métrica distinta e vive nos Indicadores, onde a vigência está disponível.
- CA-IND26: a inadimplência do mês usa a MESMA base na Revisão e nos Indicadores — a cobrança esperada da própria competência (`cobranca_esperada`, com fallback para `aluguel_esperado` do snapshot). O proxy da última receita paga só vale quando não há snapshot da competência. Antes, o mesmo fechamento exibia R$ 1.752,54 na Revisão e R$ 788,22 nos Indicadores.
...
- CA-IND27: a inadimplência do mês na Revisão é declarada pelo documento, não pelo vínculo. Linha marcada como INADIMPLÊNCIA conta mesmo sem `imovel_id` gravado: o vínculo é resolvido na leitura contra o cadastro ativo (casamento exato de código, candidato único — a mesma regra dos fluxos de gravação). O que não puder ser apurado — sem imóvel vinculado ou sem base de cálculo — aparece como pendência nominal e a métrica exibe "—", nunca R$ 0,00. Antes, exigir o vínculo zerava a métrica em silêncio e o bloco desaparecia da tela (Grand Messejana I maio/2026: 4 unidades marcadas no documento, R$ 0,00 na Revisão contra R$ 2.631,90 nos Indicadores).
- CA-IND28: dívida de competência anterior cede à rescisão. Unidade que rescindiu e pagou o proporcional dos dias ocupados encerra a competência como vaga, com a dívida residual reportada na inadimplência acumulada — nunca como inadimplência do mês derivada do aluguel integral, que a unidade já não devia (Grand Maracanaú 202, jun/2026: R$ 386,67 derivados de 400,00 − 13,33, enquanto o apto 201, mesma rescisão proporcional sem dívida listada, era classificado vago). Dívida declarada COM competência igual à do fechamento e texto explícito de inadimplência na própria linha continuam vencendo a rescisão.
- CA-IND29 (revisa CA-IND22): a inadimplência acumulada soma o que os documentos do escopo declaram e informa sobre quantos fechamentos o número fala (`declarados de total`). Anular o consolidado porque UM fechamento não traz a seção escondia o que os outros declaram — em jul/2026, R$ 56.199,25 de 5 fechamentos apareciam como "—" por causa de 3 sem a seção. Os que não declaram continuam nominados na lacuna `inadimplencia_nao_extraida`. "—" permanece apenas quando nenhum documento traz a seção e o histórico também não permite derivar (CA-IND22.1/22.2), e lista vazia com seção identificada segue sendo zero confirmado.
- CA-IND30 (revisa CA-IND21): a data do card "Hoje" é a última escrita do PRÓPRIO cadastro de imóveis, nunca o máximo entre imóveis, fechamentos e snapshots — emprestar a data das outras tabelas fazia uma posição de 12/08 anunciar-se como sincronizada em 01/09. Sem imóvel no escopo a data é desconhecida, não "agora". A barra também declara em quantas unidades a posição cadastral discorda da competência exibida, porque o cadastro só é reescrito por sincronização manual e pode ser mais antigo que o mês em tela (jul/2026: 87,2% no mês contra 94,1% no cadastro, com 21 unidades em estados diferentes).
- CA-IND31: a evolução mensal em valores mede a REALIZAÇÃO do aluguel contratado — teto contratado, recebido da competência, vacância e inadimplência — e a identidade é escrita sob a legenda (contratado − vacância − inadimplência − descontos ± ajustes = recebido), citando apenas os termos que existem no mês. As séries antigas ("Receitas", "Aluguel recebido", "Repasse") não voltam ao mesmo par de eixos: elas misturavam regime (receita por competência de origem contra repasse por caixa, R$ 2.846,67 de descasamento em maio/2026), reembolso de água/IPTU/seguro que entra e sai como despesa, e comissão — a distância vertical entre as linhas não descrevia nada.
- CA-IND32: cada ponto da série vale o que o fechamento daquele mês declara. A reatribuição por competência de origem não é somada ao valor plotado — permanece calculada em `competenciaAjuste*` e é exibida como nota do mês. Antes, maio/2026 exibia R$ 88.111,89 de receita, número que não batia com nenhum fechamento (os documentos somam R$ 85.265,22).
- CA-IND33: no modo percentual, a série é ocupação e percentual de unidades inadimplentes, ambos sobre os imóveis classificados no mês. A cobertura do dado sai do gráfico: é qualidade da extração, não indicador de operação, e já é declarada no banner de confiança.
- CA-IND34: alarme financeiro respeita a própria tolerância do projeto. A comparação com o banco arredonda a diferença antes de decidir, para que um centavo exato representado em binário (0,010000000002 em julho/2026) não acenda divergência enquanto a ponte financeira, que arredonda, considera o mês conciliado. Dois centavos continuam acendendo.
- CA-IND35: no mapa de riscos, unidade vaga não exibe nome de inquilino — nem na célula, nem no rótulo acessível, nem no agregado de empreendimento com uma única unidade. O rótulo da linha declara "Inquilino atual: <nome>" e resolve o nome pela evidência mais recente, não pelo cadastro, que só é reescrito por sincronização manual e pode estar mais antigo que o último mês processado (julho/2026: a unidade 3 do GM I exibia DANDARA, do cadastro de 07/07, enquanto o documento de julho já nomeava VICTOR). A coluna "Hoje" do mapa sai: a competência exibida já vem do filtro.
- CA-IND36: no detalhamento por imóvel, a unidade abre o histórico do imóvel com uma visão geral da competência em tela no topo — situação, inquilino, contratado, recebido, atrasos, outros recebimentos, receitas, comissão, repasse e as referências de competência e vencimento. Reaproveita o drawer de histórico existente em vez de duplicar a linha do tempo.
