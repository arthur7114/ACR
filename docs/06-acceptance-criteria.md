...
- CA-IND22 (revisado em 2026-08-28): a inadimplência acumulada nunca é exibida como R$ 0,00 quando o documento da competência não possui a seção de dívidas acumuladas. O extrato consolidado (César Rêgo/Plural — João Cordeiro, Galpão Pompílio Gomes, Galpão José Walter) não traz essa seção: a métrica fica `desconhecida` ("—") e o fechamento aparece nominalmente na lacuna `inadimplencia_nao_extraida`. Lista vazia com a seção identificada continua sendo zero confirmado (ex.: Grand Castelão maio). A ausência é detectada por `campos_ausentes` ou por `plano_extracao.secoes_identificadas` sem menção a inadimplência, o que corrige a exibição sem exigir reprocessamento. Exibir o valor real da dívida (ex.: os 2 meses do Flat B) depende de fonte própria para esse dado.
