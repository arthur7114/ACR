...
Com autorização explícita do usuário, aplicar a migration
`202608050001_contratos_locacao.sql` no Supabase (cria `contratos_locacao`,
`contrato_valores` e `lancamentos_competencia`) e rodar
`scripts/backfill-contratos.ts --apply` para popular as três tabelas a partir
do histórico existente (o dry-run já rodou e apontou contagens plausíveis de
imóveis, contratos e lançamentos). Depois rodar `scripts/verify-competencia.ts`
contra o gabarito local de junho (`tmp/gabaritos/`, não versionado, com
tolerância de R$ 0,02) e iterar nas divergências encontradas antes de avançar
