...
Status: código concluído; duas migrations pendentes de aplicação no Supabase.
...
Validação: `pnpm test` 233/233 (+7 novos), `npx tsc --noEmit` sem erros novos (persiste apenas o erro pré-existente `persist-package.ts(354)`, tolerado por `ignoreBuildErrors`), `pnpm lint` limpo. UI não exercitada em navegador (depende de fechamento aprovado + dados reais do Supabase).
...
Próxima ação: aplicar as migrations `202607280001` e `202607280002` no Supabase; configurar o dia de vencimento das regras comerciais na tela de Cadastros; validar no navegador o split (fechamento com aluguel atrasado de 2 meses), o vencimento no eGestor e o lançamento manual.
