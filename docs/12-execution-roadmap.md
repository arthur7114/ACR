...
- O modelo padrao da extracao da prestacao e `gpt-5.5` (default do agente); ele corrige a variancia do `gpt-4o` em tabela densa (GCI saiu de 1/22 para 24 linhas, repasse conciliado). `OPENAI_MODEL` e override local: o pin `gpt-4o` no `.env.local` deve ser removido para restaurar o default.

- (2026-08-28) Todo envio de PDF a OpenAI passa `detail: "high"` explicito no bloco `input_file` (`analyze-prestacao.ts`, `analyze-package-documents.ts`, `analyze-iptu.ts`). Motivo: o default `auto` resolve para `low` em modelos anteriores ao `gpt-5.6`, ou seja as imagens de pagina do `gpt-5.5` chegavam em baixa resolucao — a mesma classe de falha que fazia tabela densa perder linhas. O explicito tambem torna o comportamento independente do modelo pinado em `OPENAI_MODEL`. Custo de input por pagina sobe; acompanhar a fatura.

- (2026-08-28) Agentes leves saem de `gpt-4o-mini` (2024): classificador, repasse e reajuste vao para `gpt-5.6-luna`; despesas vai para `gpt-5.5`. Base: no leaderboard de OCR da Roboflow (34 modelos, ago/2026, similaridade media com ground truth) Luna faz 88,4% a US$ 0,0012/pagina e `gpt-5.6-terra` faz 88,8% a US$ 0,013 — +0,4 ponto por 11x o custo, entao Terra nao entra. Despesas fica em `gpt-5.5` (91,2%) por ser o material mais sujo do pacote (foto de comprovante, scan torto) e multi-item, onde cada linha perdida quebra conciliacao. `gpt-4o-mini` nao aparece nesse leaderboard, logo o delta contra ele nao esta medido — a troca se apoia na idade do modelo e no precedente do `gpt-4o` na prestacao. Falta validar contra PDFs reais em pt-BR: nenhum desses benchmarks tem documento financeiro brasileiro.
...
Proxima acao: classes A e C prevenidas em 2026-09-16 (via unica de gravacao do snapshot; constraint NULL-ou-positivo em aluguel, NOT VALID). Falta so `validate constraint` quando as tres vigencias antigas em zero (TERRENO CASTELAO, Grand Messejana II 3, LOCMAIS SALA 05) forem decididas — ate la aparecem como "sem aluguel contratado" na Visao geral. Zero fechamentos com divergencia em 4 competencias. Ocupacao acumulada implementada como proporcao de unidade-mes com janela nomeada. Valor do ativo e TERRENO CASTELAO: cliente pediu para ignorar (2026-09-16).
