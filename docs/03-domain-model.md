...
- Recebimentos extraordinários (acordo, rescisão, atraso, intermediação) têm representação canônica única resolvida por `lib/recebimentos-extraordinarios.ts`; nenhum consumidor (revisão, resumo, snapshots, indicadores, payload eGestor) recalcula comissão ou repasse por conta própria.
- A base comissionável da intermediação é a soma dos componentes comissionáveis da linha. Mapa default de comissionabilidade: aluguel e garagem comissionáveis; IPTU, água, seguro e outros encargos não. O mapa só pode ser sobreposto por imobiliária com documento/contrato explícito, nunca por inferência. Encargos compõem o total recebido e o repasse sem alterar a base percentual; a revisão exibe base, encargos, total, comissão, percentual e repasse sem conflar esses conceitos.
- Em acordo, rescisão e atraso, principal bruto, ajuste (desconto/crédito), total recebido, comissão e repasse são grandezas distintas; totais e indicadores usam o total recebido, nunca o principal.
- A base comissionável depende do tipo: intermediação incide sobre os componentes comissionáveis (aluguel + garagem); acordo, rescisão e atraso incidem sobre o total pago pelo inquilino, porque a comissão retida ali é a taxa de administração — a mesma base da comissão administrativa das linhas regulares. Juros e multas vêm embutidos na coluna do aluguel.
- Repasse embutido no próprio documento (extrato consolidado) significa comprovante bancário NÃO APLICÁVEL: esses fechamentos saem do denominador de comprovantes em vez de contar como ausência.
- Valores explícitos do documento (`total_recebido`, `comissao`, `repasse`) são autoritativos: o sistema os preserva e valida a equação `recebido − comissão = repasse` (± R$ 0,01); divergência gera pendência, não recálculo silencioso. Item sem evidência suficiente (seção, valor próprio, vínculo) fica `pendente` e fora de qualquer total confirmado — nunca vira zero.
...
- No layout César Rêgo, `SIT=ALUG` sem lançamento de ALUGUEL da competência é inadimplência explícita — inclusive quando o documento traz pagamentos de meses anteriores do mesmo imóvel (a linha da competência em aberto vem primeiro). Os meses entre `ULT. PG` e a competência, não pagos no documento, são inadimplência acumulada no valor da coluna ALUGUEL da Relação de Imóveis; `aluguel_esperado` na linha registra esse valor como base da inadimplência do mês (2026-09-02). A TED global é dividida igualmente entre os empreendimentos do extrato antes do rateio interno por imóvel. Totais, rechecks, guardrails e parecer são recalculados somente depois desse recorte; linha sem recebimento usa comissão e repasse iguais a zero na validação das colunas.
