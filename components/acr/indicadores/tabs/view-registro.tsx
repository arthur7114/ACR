  ...
  return (
    <Panel className="min-w-0 overflow-hidden">
      <PanelHeader
        ...
        action={
          <a
            ...
            download={`detalhamento-por-imovel-${data.meta.competencia}.csv`}
        ...
        <label htmlFor="indicadores-property-search" className="block min-w-0 flex-1 sm:max-w-lg">
          ...
          <span className="flex min-h-11 items-center gap-2 rounded-lg border border-acr-line-2 bg-white px-3 focus-within:border-acr-green focus-within:ring-2 focus-within:ring-acr-green/15">
            ...
            <input
              ...
              className="min-h-11 min-w-0 flex-1 bg-transparent py-2 text-sm text-acr-ink outline-none placeholder:text-acr-muted-2"
      ...
      {visible.length > 0 ? (
            ...
            <table className="min-w-[1680px] w-full border-collapse text-xs">
              ...
              <thead className="sticky top-0 z-10 bg-white">
                <tr className="border-b border-acr-line-2 text-acr-muted-2">
                  ...
                  <SortableHeader label="Receitas do fechamento" sortKey="receitaTotal" sort={sort} onSort={updateSort} right />
                  ...
                  <SortableHeader label="Repasse calculado" sortKey="repasseApurado" sort={sort} onSort={updateSort} right />
                ...
                {visible.map((row) => {
                  ...
                  return (
                    <tr key={`${row.imovelId}-${row.competencia}`} className="border-b border-acr-line last:border-0">
                      ...
                      <ReferenceCell value={references.rentCompetence} />
                      <ReferenceCell value={references.receiptCompetence} />
                      <ReferenceCell value={references.dueDay} />
        ...
        <EmptyState
          title={data.receitasPorImovel.length === 0 ? "Sem detalhamento por imóvel" : "Nenhum resultado para a busca"}
          description={data.receitasPorImovel.length === 0 ? "A tabela aparecerá quando houver histórico mensal vinculado à competência." : "Tente buscar por outra unidade, pessoa ou empreendimento."}
        />
      )}
    </Panel>
  )
...
function ReferenceCell({ value }: { value: string }) {
  return <td className="px-3 py-3 text-acr-muted-2 tabular-nums">{value}</td>
}
    ...
    <details className="group px-4 py-2">
      <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 py-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-acr-green [&::-webkit-details-marker]:hidden">
        ...
        <div className="text-right">
          <p className="text-sm font-bold text-acr-ink tabular-nums">{formatCurrency(row.aluguelRecebidoCompetencia)}</p>
          <p className="mt-0.5 text-[10px] text-acr-muted-2">Recebido da competência</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 border-t border-acr-line py-4 text-xs">
        ...
        <MobileValue label="Repasse calculado" value={formatCurrency(row.repasseApurado)} />
...
function Pagination({ page, pageCount, onPageChange }: { page: number; pageCount: number; onPageChange: (page: number) => void }) {
  return (
    <nav aria-label="Paginação do detalhamento por imóvel" className="flex items-center justify-between gap-3 border-t border-acr-line px-4 py-3 sm:px-5">
...
function buildCsvHref(rows: IndicadoresPropertyRevenue[]) {
  const header = ["competencia", "unidade", "inquilino", "empreendimento", "status", "aluguel_contratado", "modelo_receita", "recebido_competencia", "atrasos_recuperados", "outros_recebimentos", "receitas_fechamento", "desconto", "comissao_administracao", "repasse_calculado", "competencia_aluguel", "recebido_em", "vence_dia", "origem", "qualidade"]
  const lines = rows.map((row) => {
    ...
    return [
      ...
      row.modeloReceita === undefined || row.modeloReceita === "fixo"
        ? row.aluguelEsperado
        : "Não se aplica",
      row.modeloReceita ?? "",
      row.aluguelRecebidoCompetencia,
      resolveMetricValue(row.atrasosRecuperados),
      resolveMetricValue(row.outrosRecebimentos),
      row.receitaTotal,
      row.desconto,
      row.comissaoAdministracao,
      row.repasseApurado,
